...
using OwnID.Redis;
using OwnID.Server.IAS.Metrics;
using OwnID.Server.IAS.Middlewares;
using OwnID.Server.IAS.Middlewares.SecurityHeaders;
using OwnID.Web;
using OwnID.Web.IAS;
using Serilog;
...
namespace OwnID.Server.IAS
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            var iasSection = Configuration.GetSection("ias");
                ...
                x.AddPolicy(CorsPolicyName, builder =>
                {
                    ...
                    switch (serverMode)
                    {
                        ...
                        case ServerMode.Local:
                           // builder.SetIsOriginAllowed(origin => new Uri(origin).Host == "gordon-ids.sci-lab.cc.eu-de-1.cloud.sap");
                           // builder.SetIsOriginAllowed(origin => new Uri(origin).Host == "localhost");
                            originsList.Add($"http://localhost:4200");
                            originsList.Add($"https://gordon-ids.sci-lab.cc.eu-de-1.cloud.sap");
                            break;
                        default:
            ...
            {
/*                var loginTypeString = iasSection["login_type"];

                if (string.IsNullOrEmpty(loginTypeString) || !Enum.TryParse(iasSection["login_type"], true,
                    out IASLoginType loginType)) loginType = GigyaLoginType.Session;*/

                builder.UseIAS(iasSection["public_key"], iasSection["private_key"]);
                ...
                builder.WithBaseSettings(x =>
                {
                    ...
                    x.IsDevEnvironment = serverMode == ServerMode.Local || serverMode.Equals("ias");
                });
